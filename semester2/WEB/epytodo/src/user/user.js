...
router.get('/user', auth, async (req, res) => {
    ...
    const [result, fields] = await global.connection.query(
        `SELECT *, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS created_at FROM user WHERE email = ?`,
        [req.user.email]
    );
    if (result.length <= 0) {
...
router.get('/user/todos', auth, async (req, res) => {
    ...
    const [user_todos] = await global.connection.query(
        `SELECT *, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS created_at, DATE_FORMAT(due_time, '%Y-%m-%d %H:%i:%s') AS due_time FROM todo WHERE user_id = ?`,
        [user_id]
    );
    return res.status(200).json(user_todos);
});

router.get('/users/:target', auth, async (req, res) => {

    const [result, fields] = await global.connection.query(
        `SELECT *, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS created_at FROM user WHERE email = ? or id = ?`,
...
router.delete('/users/:id', auth, async (req, res) => {
    ...
    const [result, fields] = await global.connection.query(
        `SELECT *, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') AS created_at FROM user WHERE id = ?`,
        [req.params.id]
    );
    if (result.length <= 0) {
        return res.status(404).json({msg: "Not found"})
